...
public class ItemPickup : MonoBehaviour
{
    ...
    public int amount;
    public bool bonus;
    public WeaponType givesWeapon = WeaponType.None;
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (givesWeapon != WeaponType.None)
        {
            if (givesWeapon >= 0 && (int)givesWeapon < PlayerInfo.Instance.Weapon.Length)
            {
                if (!PlayerInfo.Instance.Weapon[(int)givesWeapon])
                {
                    PlayerInfo.Instance.Weapon[(int)givesWeapon] = true;
